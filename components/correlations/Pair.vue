...
const { candles: candlesAsset1, pending: pendingAsset1 } =
  await useBinanceFuturesChartData({
    symbol: props.asset1,
    interval: "1h",
    startTime,
  });

const { candles: candlesAsset2, pending: pendingAsset2 } =
  await useBinanceFuturesChartData({
    symbol: props.asset2,
    interval: "1h",
    startTime,
...
const relativeValues = computed(() => {
  ...
  for (let i = 1; i < candlesAsset1.length; i++) {
    const prevCandle1 = candlesAsset1[i - 1];
    const candle1 = candlesAsset1[i];
    price1 += calcRelative(prevCandle1.close, candle1.close);

    const prevCandle2 = candlesAsset2[i - 1];
    const candle2 = candlesAsset2[i];
